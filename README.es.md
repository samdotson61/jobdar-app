...
`jobdar` es un solo comando con subcomandos sencillos — y `jd` es su alias corto (`jd scan` ≡ `jobdar scan`):
...
- **Flujo descubrir → prefiltrar → evaluar** — `scan` encuentra y filtra puestos pero **nunca los puntúa**; `jobdar prescreen` descarta requisitos duros (años exigidos, autorización de seguridad activa, título obligatorio) **con una razón citada — nunca en silencio** — y ordena el resto por coincidencia de habilidades + frescura; el modelo (`jobdar eval`) puntúa la compatibilidad **0–5** con tu currículum y registra una banda **Postular / Investigar / Descartar**. `jobdar tui` muestra los puestos descubiertos como *pendiente eval* hasta que el modelo los puntúa. **Alcance honesto:** la puntuación compara el *texto del anuncio* con tu currículum — Jobdar no verifica al empleador, y cada informe de evaluación lo dice; `jobdar recheck` verifica que los puestos puntuados sigan publicados (los retirados muestran "ya no publicado" en vez de una insignia verde) — y desde 1.60.0 las vistas verifican por sí mismas: `tui`, `dashboard` y `report` comprueban automáticamente cualquier puesto Postular/Investigar sin verificación del día antes de mostrarlo (`--no-verify` lo omite, p. ej. sin conexión), así un puesto retirado nunca se presenta como oportunidad — y `jobdar feedback` recoge tus 👍/👎 para que `calibrate --feedback` mida — no adivine — cuántas veces acierta el evaluador.
