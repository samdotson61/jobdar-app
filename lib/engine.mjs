...
export async function evaluate({ active, jd, cv = '', profile = {}, today = '', confirm = false, escalate = null, transferable }) {
  if (confirm) {
    const pc = await preConfirm({ active, jd, cv, profile, transferable })
    if (pc.verdict === 'skip') return { ok: true, skipped: true, preConfirm: pc }
  }
  let v = await evalRole({ active, jd, cv, profile, today, transferable })
  if (escalate && v.ok && isBorderline(v) && escalate.up && escalate.kind !== active.kind) {
    const v2 = await evalRole({ active: escalate, jd, cv, profile, today, transferable })
...
export async function tailor({ active, jd, cv = '', profile = {}, role = '', company = '' }) {
  const r = await tailorRole({ active, jd, cv, profile, role, company })
  if (!r.ok) return r
