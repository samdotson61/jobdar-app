...
export function extractCredential(jd) {
  ...
  for (const c of HARD_CREDENTIALS) {
    const m = text.match(c.jd)
    if (!m) continue
    const around = text.slice(Math.max(0, m.index - 70), m.index + m[0].length + 70)
    if (SOFT_CTX.test(around)) continue
    if (REQ_CTX.test(around)) return { gate: 'required', credential: c.id, quote: clip(around) }
  }
...
const FIELD_TITLE = [
  { field: 'accounting', re: /\b(accountant|accounting|auditor|controller|comptroller|bookkeeper|tax (?:associate|accountant|preparer|manager|analyst)|accounts (?:payable|receivable) (?:specialist|clerk|manager|associate))\b/i },
