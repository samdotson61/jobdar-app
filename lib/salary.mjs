// Jobdar — deterministic salary extraction + lenient target banding (Phase 7.8.1; rec-spec §1).
...
const MONEY = String.raw`\$?\s?(\d{1,3}(?:,\d{3})+(?:\.\d{1,2})?|\d{1,3}(?:\.\d{1,2})?)\s?([kK])?`
const RANGE_SRC = `${MONEY}\\s*(?:-|–|—|to|through)\\s*${MONEY}`
const SINGLE_SRC = `\\$\\s?(\\d{1,3}(?:,\\d{3})+(?:\\.\\d{1,2})?|\\d{1,3}(?:\\.\\d{1,2})?)\\s?([kK])?`
