// Jobdar — the PURE pipeline logic (Phase 10 split). Everything here is arrays/strings in → out, no fs,
...
// @jobdar/engine. Semantics are UNCHANGED — this is a move, not a rewrite (test-all.mjs guards it).
...
// Older pipeline files read fine (missing cols → '').
export const PIPELINE_COLS = ['company', 'role', 'url', 'location', 'score', 'band', 'recommendation', 'status', 'posted', 'first_seen', 'updated', 'prescreen', 'screen_reason', 'pay', 'aliases', 'notes']
...
export function mergeScanned(existing, discovered, dateStr) {
  ...
  for (const j of discovered || []) {
    ...
    const fresh = {
      ...
      prescreen: '', screen_reason: '', pay: '', aliases: '', notes: '',
    }
    byUrl.set(url, fresh)
