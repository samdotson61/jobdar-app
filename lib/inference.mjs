...
export function parseVerdict(text) {
  ...
  // Bounded \s{0,4} (not \s*) so a long unterminated whitespace run can't trigger O(n²) backtracking.
  const m = s.match(new RegExp(`fit\\s{0,4}score\\s{0,4}[:=]?\\s{0,4}(\\d+(?:\\.\\d+)?)${TAG}`, 'i')) ||
            s.match(new RegExp(`\\bscore\\s{0,4}[:=]\\s{0,4}(\\d+(?:\\.\\d+)?)${TAG}`, 'i'))
  let score = null
  if (m) {
    const n = Number(m[1])
