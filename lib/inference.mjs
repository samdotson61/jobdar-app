...
// gitignored data/credentials.env at startup (bin/jobdar) so `jobdar init`'s saved key still works.
...
export function resolveBackend(profile = {}, env = process.env) {
  ...
  return { mode, runtime, protocol: rt.protocol, healthPath: rt.health, localUrl, apiUrl, apiModel, localModel, apiKey: env.JOBDAR_API_KEY || '' }
}
...
export async function selectActive(profile = {}, env = process.env) {
  ...
  // All local runtimes do — winc (jobdar.4+), ollama, llamafile; the Anthropic api is Messages-only.
...
export async function submitBatch(active, requests, { pollMs = 10000, maxWaitMs = 1800000, onPoll = null } = {}) {
  ...
  const okJson = async (res, what) => {
    ...
    try { return await res.json() } catch { throw new Error(`Batch ${what}: response was not JSON`) }
  }
  // Every hop is bounded (1.63.1): an unanswered create/poll used to hang the run forever.
  const timed = async (url, init, ms, what) => {
    const ctrl = new AbortController()
    const timer = setTimeout(() => ctrl.abort(), ms)
    try { return await fetch(url, { ...init, signal: ctrl.signal }) } catch (e) {
      throw new Error(`Batch ${what}: ${e && e.name === 'AbortError' ? `no response within ${Math.round(ms / 1000)}s` : (e && e.message) || e}`)
    } finally { clearTimeout(timer) }
  }
  const create = await okJson(await timed(`${base}/v1/messages/batches`, { method: 'POST', headers, body: JSON.stringify({ requests }) }, 60000, 'create'), 'create')
  ...
  while (status.processing_status !== 'ended') {
    ...
    status = await okJson(await timed(`${base}/v1/messages/batches/${create.id}`, { headers }, 30000, 'poll'), 'poll')
  }
  if (!status.results_url) throw new Error(`Batch ${create.id} ended with no results_url`)
  const rres = await timed(status.results_url, { headers }, 300000, 'results')
