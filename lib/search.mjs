...
// How relevant is `text` (a role title / "title company location") to the search terms?
//   < 0  → an exclude term hit (cut it outright)
//   0    → no overlap (not relevant to what the user asked for)
//   > 0  → relevant; higher = a title-phrase match or more keyword hits
export function relevanceScore(text, terms) {
  ...
  let score = 0
  for (const tt of titles) if (tt && hay.includes(String(tt).toLowerCase())) score += 3
  for (const k of keywords) if (k && new RegExp(`\\b${escapeRe(String(k).toLowerCase())}`).test(hay)) score += 1
  return score
}
