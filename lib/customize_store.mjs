// Jobfaro — customization state (Phase 8f). Persists, per role and per artifact (`tailor` | `outreach`),
...
// `extra` lets a caller fold in inputs beyond cv/jd/directives (outreach mixes in channel + recipient);
// it's only added to the payload when non-empty, so tailor hashes stay identical across this upgrade.
export function contentHash(cv, jd, directives, extra = '') {
  const payload = { cv: String(cv || ''), jd: String(jd || ''), directives: directives || [] }
  if (extra) payload.extra = String(extra)
