// Jobfaro — the PURE outreach logic (Phase 10 split): cadence rules, people-finder links,
// business-day math, draft lint, and the grounded model-drafting verb. No fs — the ledger I/O
// (readOutreach/appendOutreach) stays in lib/outreach.mjs, which re-exports this whole surface.
// Exported to the native/web apps via @jobfaro/engine so on-device outreach runs the identical rules.

import { callBackend } from './inference.mjs'
import { stripPII, parseEvalJson } from './eval_engine.mjs'
import { directiveBlock, fillSignature } from './tailor.mjs'
...
export function businessDaysBetween(fromIso, toIso) {
  // Step in UTC so a calendar day is exactly 24h regardless of the machine's DST transitions — stepping
  // in local time over-counted by one across spring-forward week and fired a follow-up a day early.
  const parse = (s) => { const m = String(s || '').match(/^(\d{4})-(\d{2})-(\d{2})/); return m ? Date.UTC(+m[1], +m[2] - 1, +m[3]) : NaN }
...
export function lintDraft(text, { channel = 'linkedin', person = '' } = {}) {
  ...
  // Also catch the stripPII sentinels ([email]/[phone]/[url]) a small model might echo from the résumé.
  const placeholder = body.match(/\{[a-z0-9_ ]+\}|\[(?:name|company|role|email|phone|url)[^\]]*\]/i)
...
export async function draftOutreach({ active, jd = '', cv = '', profile = {}, role = '', company = '', person = '', channel = 'linkedin', directives = [], maxTokens = 400, timeoutMs = 120000 }) {
  ...
  // fillSignature: if a small model leaves the candidate's own "[name]" in a sign-off (stripPII removed it
  // from the résumé), restore it before lint so the sender signature isn't false-flagged as a placeholder —
  // the recipient name + company/role are given in the prompt, so any leftover [name] is the sender's.
  const pick = (txt) => { const j = parseEvalJson(txt); return fillSignature((j && typeof j.message === 'string' ? j.message : String(txt || '')).trim(), profile.name) }
