// Jobdar — region taxonomy & location filtering (Phase 5).
...
export const REGIONS = {
  ...
  northeast: {
    ...
    metros: ['New York', 'Boston', 'Philadelphia', 'Pittsburgh', 'Baltimore', 'Newark', 'Providence', 'Hartford'],
  },
  southeast: {
...
export function parseLocation(text) {
  ...
  for (const c of raw.match(/\b[A-Z]{2}\b/g) || []) if (STATE_CODES.has(c)) usStates.add(c)
  for (const [name, code] of STATE_NAME_ENTRIES) if (lower.includes(name)) usStates.add(code)
...
export function canonicalLocation(text) {
  const raw = String(text || '')
  const lower = raw.toLowerCase()
  for (const [metro] of METRO_ENTRIES) if (lower.includes(metro)) return metro
  for (const c of raw.match(/\b[A-Z]{2}\b/g) || []) if (STATE_CODES.has(c)) return c
  for (const [name, code] of STATE_NAME_ENTRIES) if (lower.includes(name)) return code
