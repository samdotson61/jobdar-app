...
//   POST /import      {file}                            extract+structure a résumé
...
import { loadProfile, loadCv, loadPortals, paths, atomicWrite } from '../config.mjs'
...
import {
  ...
  upsertScanned, upsertPrescreen, upsertEval, updateStatusByUrl,
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'POST' && path === '/prescreen') {
        ...
        let rows = readPipeline().filter((r) => r.url && !isEvaluated(r) && !isTracked(r) && !String(r.prescreen || '').trim())
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          upsertPrescreen(row.url, { score, reason, pay }, date)
          v.screened || dead ? screened++ : ranked++
          if (i < rows.length - 1) await sleep(200) // polite pacing between live JD fetches
        }
