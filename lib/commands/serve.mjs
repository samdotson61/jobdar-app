  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'POST' && path === '/profile') {
        ...
        return send(res, 200, { ok: true, saved: { name: s.name || '', target_regions: s.target_regions || [], target_levels: s.target_levels || [], target_salary: s.target_salary || 0, transferable_skills: Boolean(s.transferable_skills), language: s.language || 'en' } }, req)
      }
      ...
      if (req.method === 'POST' && path === '/prescreen') {
        ...
        const prof = Number.isFinite(Number(p.targetSalary)) ? { ...profile, target_salary: Number(p.targetSalary) } : profile
        const date = today()
      ...
      if (req.method === 'POST' && path === '/evaluate') {
        ...
        const evalProfile = Number.isFinite(Number(p.targetSalary)) ? { ...profile, target_salary: Number(p.targetSalary) } : profile
        const v = await evaluate({ active: a, jd, cv: p.cv || loadCv(), profile: evalProfile, today: today(), confirm: Boolean(p.confirm), transferable: p.transferable })
      ...
      // Eval feedback (thumbs up/down on a verdict) → the local labeled set `jobdar calibrate --feedback` reads.
