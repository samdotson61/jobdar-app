// Jobfaro — `jobfaro pdf` (career-ops "Customize" stage). Renders your cv.md into a tailored,
...
//   jobfaro pdf                 # render your résumé
//   jobfaro pdf Enova           # tailor to the matching pipeline role
...
export async function runPdf(argv = []) {
  ...
  // Opt-in PDF: Playwright stays optional and is lazy-imported only on demand. The headless render
  // is a few silent seconds — the sweep keeps it visibly alive (elapsed time, no fake percent).
  const sweep = createRadar({ total: null })
  const pdfPath = path.join(paths.outputDir, base + '.pdf')
  try {
    const { chromium } = await import('playwright')
    sweep.start(base + '.pdf')
