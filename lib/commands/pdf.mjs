...
export async function runPdf(argv = []) {
  ...
  console.log(color.green(t('pdf.html_written', { file: path.relative(paths.root, htmlPath) })))
  ...
  try {
    ...
    try {
      ...
      console.log(color.green(t('pdf.pdf_written', { file: path.relative(paths.root, pdfPath) })))
    } finally {
      await browser.close()
    }
  } catch {
