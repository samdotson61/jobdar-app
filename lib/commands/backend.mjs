// Jobdar — `jobdar backend` (Phase 8b). Manage the on-device inference backend (winc.cpp).
//   jobdar backend            status: which backend, URL, and whether it's live
//   jobdar backend --check    canary: GET /health + one real eval round-trip (the verification gate)
//   jobdar backend --install  bootstrap winc.cpp (delegated to winc; we orchestrate + verify)
...
// Minimum winc for a working install path: llama.cpp changed its release scheme on 2026-08-21, and any
// winc older than 1.40.0 can no longer fetch an engine (`winc setup` fails). Compare by the base X.Y.Z —
// the branch's `-jobdar.N` suffix (or any prerelease tag) is ignored. Returns -1/0/1, or null when either
// side has no parseable X.Y.Z (an unknown build is neither proven old nor new — callers don't block on it).
export const MIN_WINC = '1.40.0'
export function compareWincVersion(a, b) {
  const parse = (v) => {
...
// Bootstrap winc.cpp. We DELEGATE to winc (never reimplement model download/serve); jobdar orchestrates
...
async function install(t, r) {
  ...
  // Too-old winc can't fetch an engine any more — `winc setup` would fail downstream. Say so up front
  // with the fix instead of letting the user discover it mid-install. An unparseable version passes.
  if (compareWincVersion(winc.version, MIN_WINC) === -1) {
    console.log(color.yellow(t('backend.winc_too_old', { version: winc.version, min: MIN_WINC })))
    console.log('  ' + t('backend.winc_update_hint'))
    process.exitCode = 1
    return { installed: false, tooOld: true, version: winc.version }
  }
