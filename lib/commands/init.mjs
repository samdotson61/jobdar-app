// Jobfaro — `jobfaro init` (Phase 6.1). Bilingual interactive setup wizard. Asks a few questions,
// then writes config/profile.yml AND materializes config/portals.yml from the region seed catalog —
// so `jobfaro scan` works immediately, no YAML editing. `--defaults`/`--yes` runs non-interactively;
// any field can be preset with a flag (--region, --levels, --name, --resume, …).

import { mkdirSync } from 'node:fs'
import path from 'node:path'
import yaml from 'js-yaml'
import { paths, PROFILE_DEFAULTS, SUPPORTED_LANGUAGES, atomicWrite } from '../config.mjs'
...
function saveApiKey(key) {
  ...
  atomicWrite(path.join(paths.dataDir, 'credentials.env'), `JOBFARO_API_KEY=${key}\n`, { mode: 0o600 })
}
...
export async function runInit(argv = []) {
  ...
  // Transferable-skills toggle — defaults ON for career changers / no-degree (they rely on it most).
  const xferDefault = ['career_changer', 'no_degree'].includes(tuning) ? 'yes' : 'no'
  // Tri-state from the flag VALUE: a bare `--transferable` is `true`; `--transferable no`/`=false`/`=0`
  // must read as "no" (the old `flags.transferable ? …` coerced every non-empty string to truthy → "yes").
  const xferFlag = flags.transferable == null ? null
    : (flags.transferable === true || /^(1|yes|true|on)$/i.test(String(flags.transferable))) ? 'yes' : 'no'
  const transferableSel = await pickOne(xferFlag, 'init.q_transferable', [
    { value: 'yes', label: t('init.xfer_yes') },
    { value: 'no', label: t('init.xfer_no') },
  ], xferDefault)
  ...
  const profile = {
    ...
    transferable_skills: transferableSel === 'yes',
    inference,
    name: (resumeInfo && resumeInfo.name) || name,
    location: (resumeInfo && resumeInfo.location) || location,
    target_salary,
  }
  mkdirSync(paths.configDir, { recursive: true }) // first run may target a fresh JOBFARO_HOME / ~/.jobfaro
  atomicWrite(paths.profile, yaml.dump(profile, { lineWidth: 100 }))

  const portals = toPortals(selectEmployers({ regions: [region] }))
  if (portals.length) atomicWrite(paths.portals, yaml.dump({ portals }, { lineWidth: 100 }))
