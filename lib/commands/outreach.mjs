...
export async function runOutreach(argv = []) {
  ...
  if (flags.draft) {
    ...
    let jd = ''
    if (jdFlag) { const d = await resolveJd(jdFlag); jd = d.description || ''; role = role || d.title || '' }
    else {
    ...
    if (!jd) { console.log(color.yellow(t('outreach.draft_no_jd'))); return { ok: false } }
    ...
    const r = await engineDraftOutreach({ active, jd, cv, profile, role, company, person, channel, directives })
