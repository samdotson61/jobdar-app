...
import { evalRole, preConfirm, prepEval, buildVerdict, EVAL_SYSTEM, EVAL_JSON_SCHEMA } from '../eval_engine.mjs'
...
async function runAutoEval(flags, positionals, t) {
  ...
  if (flags.batch && active.kind === 'api' && targets.length > 1) return runBatchEval({ targets, active, profile, cv, today, t })
  if (flags.batch) console.log(color.dim('  ' + t('eval.batch_skipped'))) // requested but not eligible → run live

  // 8a.4a: use guaranteed-JSON only on an OpenAI-compat backend that opted in; else robust prompt-parse.
  const responseFormat = profile.eval_grammar && active.protocol === 'openai' ? { type: 'json_schema', json_schema: EVAL_JSON_SCHEMA } : null
  let evaluated = 0
  let failed = 0
  let skipped = 0
  let first = true
  for (const row of targets) {
    ...
    try {
      const v = await evalRole({ active, jd, cv, profile, today, responseFormat })
      if (!v.ok) {
        console.log('  ' + color.yellow(t('eval.auto_unparsed', { role: row.role || row.url })))
        failed++
        continue
      }
      upsertEval({ url: row.url, score: v.score, band: v.band, company: row.company, role: row.role, location: row.location, recommendation: v.recommendation }, today)
      if (v.clamped) logClamp(clampLogEntry(v, row), today) // 8a.5: persist the override (model vs gate) for drift tracking
      evaluated++
      const tag = v.clamped ? color.yellow(' (' + t('eval.auto_clamped') + ')') : ''
      console.log(`  ${color.green('✓')} ${v.score.toFixed(1)} ${t('bands.' + v.band)}${tag}  ${row.company} — ${row.role}${v.pay ? color.dim('  ' + v.pay) : ''}`)
    } catch (e) {
...
async function runBatchEval({ targets, active, profile, cv, today, t }) {
  const prepped = []
  let noJd = 0
  for (const row of targets) {
    let jd = ''
  ...
  const requests = buildBatchRequests(prepped.map((p) => ({ custom_id: p.custom_id, user: p.user })), { model: active.model, system: EVAL_SYSTEM })
  let results
  try {
  ...
  for (const p of prepped) {
    ...
    const v = buildVerdict({ text: r.text, jd: p.jd, gates: p.gates, decision: p.decision, profile, usage: r.usage, model: active.model, backend: 'api' })
    if (!v.ok) continue
