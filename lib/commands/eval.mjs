...
import { evalRole, preConfirm, isBorderline, prepEval, buildVerdict, evalSystemFor, EVAL_JSON_SCHEMA } from '../eval_engine.mjs'
...
async function runAutoEval(flags, positionals, t) {
  ...
  for (const row of targets) {
    ...
    try {
      const v = await evalRole({ active, jd, cv, profile, today, responseFormat, transferable })
      if (!v.ok) {
