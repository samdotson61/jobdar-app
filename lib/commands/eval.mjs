...
import { evalRole, preConfirm, isBorderline, prepEval, buildVerdict, EVAL_SYSTEM, EVAL_JSON_SCHEMA } from '../eval_engine.mjs'
      ...
      if (flags.escalate && isBorderline(v)) {
        try {
          ...
          if (esc.up && esc.kind !== active.kind) {
            const v2 = await evalRole({ active: esc, jd, cv, profile, today })
            if (v2.ok) {
