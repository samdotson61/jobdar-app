// Jobdar — `jobdar prescreen`. The zero-token apply-likelihood gate (lib/prescreen.mjs) between scan
...
export async function runPrescreen(argv = []) {
  ...
  for (const row of rows) {
    ...
    const verdict = prescreenRole({ jdText, cvText: cv, posted: row.posted, firstSeen: row.first_seen, today, profile })
    const reason = verdict.screened ? reasonLine(verdict.reasons, t) : ''
    upsertPrescreen(row.url, { score: verdict.score, reason }, today)
  ...
  if (ranked.length) {
    ...
    for (const { row, verdict } of ranked) {
      const marks = verdict.flags.map((f) => t(`prescreen.flag_${f.kind}`)).join(', ')
      const tail = [verdict.jdAvailable ? '' : t('prescreen.jd_unavailable'), marks].filter(Boolean).join(' · ')
      console.log(`  ${String(verdict.score).padStart(3)}  ${row.company} — ${row.role}${tail ? color.dim(`  (${tail})`) : ''}`)
    }
  }
  if (screened.length) {
