// Jobfaro — `jobfaro tailor <company|url|file>` (Apply-stage "Customize"). Model-backed: produces a
...
//   jobfaro tailor Enova                                    # tailor (reuses any stored directives)
//   jobfaro tailor Enova --instruct "warmer, one paragraph shorter"
//   jobfaro tailor Enova --list                             # show stored directives + current variant
//   jobfaro tailor Enova --reset                            # drop stored directives, start clean
...
// Every pipeline row matching a tailor target — exact --url first, else company/role substring.
// Ordered as stored; the first is used, the rest become suggestions when its posting turns out dead.
export function findRoleMatches(rows, { url, query }) {
  if (url) {
  ...
  if (!q) return []
  return rows.filter((r) => String(r.company || '').toLowerCase().includes(q) || String(r.role || '').toLowerCase().includes(q))
}
...
export async function runTailor(argv = []) {
  ...
  if (jdFlag) {
    const d = await resolveJdSafe(jdFlag, t); jd = d.description; role = role || d.title || ''; jdFetchFailed = !d.ok
  } else {
    const matches = findRoleMatches(readPipeline(), { url: typeof flags.url === 'string' ? flags.url : '', query: target })
    const row = matches[0]
    if (row) {
      role = role || row.role || ''; company = company || row.company || ''; url = url || row.url || ''
      const d = await resolveJdSafe(row.url, t)
      jd = d.description
      jdFetchFailed = !d.ok
      // The matched posting is gone but the query fits other pipeline rows → hand the user a way on.
      if (!d.ok && matches.length > 1) {
        console.log(color.dim('  ' + t('common.jd_alternatives', { count: matches.length - 1 })))
        for (const o of matches.slice(1, 4)) console.log(color.dim(`    • ${o.company} — ${o.role}\n      jobfaro tailor --url ${o.url}`))
      }
    } else if (isExtractable(target) && existsSync(target)) {
      const d = await resolveJdSafe(target, t); jd = d.description; role = role || d.title || ''; jdFetchFailed = !d.ok
    }
  }
  ...
  if (!jd) {
    if (!jdFetchFailed) console.log(color.yellow(t('tailor.no_jd'))) // fetch failures already explained themselves
    process.exitCode = 1
    return { ok: false }
  }
  ...
  // One open-ended model call → the honest display is the bouncing sweep + true elapsed time.
  const sweep = createRadar({ total: null })
  sweep.start(role || company || target)
  let r
  try {
    r = await engineTailor({ active, jd, cv, profile, role, company, directives })
  } finally {
    sweep.stop()
  }
