// Jobdar — `jobdar tui`. An interactive, zero-dependency terminal dashboard over the SCORED pipeline
// (data/pipeline.tsv, written by `jobdar scan`). Roles are color-coded by band — Apply (green) /
// Research (yellow) / Don't (dim) — sortable and filterable. renderTui() is pure (unit-tested);
// runTui() drives the raw-mode loop. Non-TTY → one frame and exit (scriptable).
//   Keys: s sort · 1/2/3 band filter · 0 all · c cycle company · C clear · r refresh · q quit · ↑/↓ scroll
...
const num = (r) => Number(r.composite) || 0
const pad = (s, n) => {
...
function view(rows, { sort, filter, company }) {
  let v = rows.slice()
  if (filter && filter !== 'all') v = v.filter((r) => r.band === filter)
  ...
  else v.sort((a, b) => num(b) - num(a)) // score (band ≈ score)
  return v
}
...
export function renderTui(t, { profile, rows = [], lang, sort = 'score', filter = 'all', company = null, scroll = 0, height = 24 }) {
  ...
  for (const r of rows) if (counts[r.band] != null) counts[r.band]++
  ...
  lines.push(
    ' ' +
      ...
      (filter !== 'all' ? color.cyan(`  [${t('bands.' + filter)}]`) : '') +
  ...
  if (!v.length) {
    ...
    lines.push(color.dim('   ' + pad(t('tui.col_score'), 6) + pad(t('tui.col_band'), 11) + pad(t('tui.col_role'), 30) + pad(t('tui.col_company'), 16) + t('tui.col_location')))
    const room = Math.max(3, height - 13)
    const shown = v.slice(scroll, scroll + room)
    for (const r of shown) {
      const c = BAND_COLOR[r.band] || ((s) => s)
      lines.push('   ' + c(pad(num(r).toFixed(1), 6) + pad(t('bands.' + r.band) || r.band, 11)) + pad(r.role, 30) + pad(r.company, 16) + (r.location || ''))
    }
