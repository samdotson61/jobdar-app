...
export async function preConfirm({ active, jd, cv = '', profile = {}, maxTokens = 80, timeoutMs = 60000 }) {
  const user = `Candidate (résumé excerpt):\n${stripPII(String(cv || '').slice(0, 1500), profile)}\n\nJob:\n${String(jd || '').slice(0, 2000)}`
  const { text, usage } = await callBackend(active, { system: PRECONFIRM_SYSTEM, user, maxTokens, timeoutMs })
