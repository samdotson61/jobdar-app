...
export const EVAL_SYSTEM =
  ...
  'ONLY fit against the résumé — do NOT consider salary. For each criterion, reason briefly then rate it on ' +
  'a 5-level scale: "strong" (clearly meets), "good" (mostly meets), "partial" (partial or transferable), ' +
  '"weak" (a little), or "none" (NO relevant signal at all — use rarely; do NOT use "none" merely because ' +
  'the candidate is below the target, only when there is genuinely nothing to point to), with a short ' +
  'quoted JD line as evidence. Use the VERIFIED REQUIREMENTS provided to set candidate_meets_all (true only if the ' +
...
// excluded-degree / hard-identity field) still clamp even when the note mentions a year count. Pure.
...
export function prepEval({ jd, cv = '', profile = {}, today = '', title = '' }) {
...
export async function evalRole({ active, jd, cv = '', profile = {}, today = '', maxTokens = 700, timeoutMs = 120000, responseFormat, transferable }) {
  const xfer = transferable === undefined ? Boolean(profile.transferable_skills) : transferable
  ...
  let res
  try {
    res = await callBackend(active, { system: evalSystemFor(xfer), user, maxTokens, timeoutMs, responseFormat: rf, cache: true })
  } catch (e) {
    if (!rf) throw e
    res = await callBackend(active, { system: evalSystemFor(xfer), user, maxTokens, timeoutMs, responseFormat: null, cache: true }) // degrade to /v1/messages on a JSON-path error
  }
