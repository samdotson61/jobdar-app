// Jobfaro — configuration & paths.
// Loads the user profile and portal list, applies defaults, and resolves all
// on-disk locations. Honors JOBFARO_* environment overrides (see docs / Phase 0.3).
...
//   1. JOBFARO_HOME env (relocate everything with one variable; copy that folder to migrate)
...
//   3. ~/.jobfaro — the default for global installs (npm -g), so user data NEVER lives inside
//      node_modules where an update would wipe it
// JOBFARO_CONFIG_DIR / JOBFARO_DATA_DIR / JOBFARO_OUTPUT_DIR still override individual dirs.
const env = process.env
const repoLocal = existsSync(path.join(ROOT, 'config', 'profile.yml'))
const HOME_BASE = env.JOBFARO_HOME ? path.resolve(env.JOBFARO_HOME) : repoLocal ? ROOT : path.join(os.homedir(), '.jobfaro')
const CONFIG_DIR = env.JOBFARO_CONFIG_DIR ? path.resolve(env.JOBFARO_CONFIG_DIR) : path.join(HOME_BASE, 'config')
const DATA_DIR = env.JOBFARO_DATA_DIR ? path.resolve(env.JOBFARO_DATA_DIR) : path.join(HOME_BASE, 'data')
...
export const paths = {
  ...
  outputDir: env.JOBFARO_OUTPUT_DIR ? path.resolve(env.JOBFARO_OUTPUT_DIR) : path.join(HOME_BASE, 'output'),
}
...
export const PROFILE_DEFAULTS = {
  ...
  // silent (most) are untouched — Jobfaro never claims a stance the employer didn't state.
  needs_sponsorship: false,
  target_salary: 0,
  score_weights: { resume: 0.7, location: 0.15, seniority: 0.1, salary: 0.05 },
}

// The BYO-key for the `api`/`auto` backends. Never in the tracked profile — `jobfaro init` writes it to
// the gitignored data/credentials.env (0600). Env JOBFARO_API_KEY wins (CI / one-off overrides).
export function loadApiKey(e = env) {
  if (e.JOBFARO_API_KEY) return e.JOBFARO_API_KEY
  const file = path.join(DATA_DIR, 'credentials.env')
  if (!existsSync(file)) return ''
  const m = readFileSync(file, 'utf8').match(/^\s*JOBFARO_API_KEY\s*=\s*(.+?)\s*$/m)
  return m ? m[1].trim() : ''
}
...
function readYaml(file) {
  ...
  try {
    ...
    const e = new Error(`${file} is not valid YAML — ${err.reason || err.message}. Fix it by hand, or re-run \`jobfaro init\` to regenerate it.`)
    e.userFacing = true
    throw e
  }
}
