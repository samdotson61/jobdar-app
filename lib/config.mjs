...
// 1.63.0 compat (Jobfaro → Jobdar revert): the 2026-07-16 → 2026-09-25 interlude used JOBFARO_* names and
// ~/.jobfaro. Honor a stale JOBFARO_* export by mirroring it onto the JOBDAR_* name it now means — once,
// loudly on stderr — so nothing silently loses its home or key. Removed in 1.64 (nothing shipped publicly).
export const legacyEnvHonored = []
for (const k of Object.keys(env)) {
  if (!k.startsWith('JOBFARO_')) continue
  const nk = 'JOBDAR_' + k.slice('JOBFARO_'.length)
  if (env[nk] === undefined) { env[nk] = env[k]; legacyEnvHonored.push(`${k}→${nk}`) }
}
if (legacyEnvHonored.length && !env.JOBDAR_QUIET) {
  process.stderr.write(`[jobdar] legacy env honored: ${legacyEnvHonored.join(', ')} — rename to JOBDAR_* (support ends in 1.64)\n`)
}

const repoLocal = existsSync(path.join(ROOT, 'config', 'profile.yml'))
const DEFAULT_HOME = path.join(os.homedir(), '.jobdar')
const LEGACY_HOME = path.join(os.homedir(), '.jobfaro')
// Legacy-home fallback: an existing ~/.jobfaro is used (and named by `jobdar doctor`) until it is moved.
const useLegacyHome = !env.JOBDAR_HOME && !repoLocal && !existsSync(DEFAULT_HOME) && existsSync(LEGACY_HOME)
const HOME_BASE = env.JOBDAR_HOME ? path.resolve(env.JOBDAR_HOME) : repoLocal ? ROOT : useLegacyHome ? LEGACY_HOME : DEFAULT_HOME
export const legacyHomeInUse = useLegacyHome
...
export const PROFILE_DEFAULTS = {
  ...
  // When on, roles whose JD explicitly refuses visa sponsorship (or requires US citizenship) are screened
  // out with the line quoted; roles that explicitly OFFER sponsorship get a fit indicator. JDs that are
  // silent (most) are untouched — Jobdar never claims a stance the employer didn't state.
...
export function loadApiKey(e = env) {
  ...
  // JOBFARO_API_KEY= is the 1.49–1.62 spelling `init` wrote; still read (compat, gone in 1.64).
  const m = readFileSync(file, 'utf8').match(/^\s*JOB(?:DAR|FARO)_API_KEY\s*=\s*(.+?)\s*$/m)
  return m ? m[1].trim() : ''
}
