// Jobfaro — tiny interactive prompt helper (zero deps, node:readline). Used by `jobfaro init`.
...
export async function ask(question, def = '') {
  ...
  // At a real terminal, drop any type-ahead / stray newline so it can't skip this prompt. Piped
  // input (not a TTY) keeps the queue, since there every buffered line is a real answer.
  if (process.stdin.isTTY) queue.length = 0
