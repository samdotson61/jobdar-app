...
// `prescreen` (0–100) + `screen_reason` are written by `jobdar prescreen` (lib/prescreen.mjs): the
// zero-token gate that ranks the eval queue and screens hard-gated roles — with the reason kept on
// the row so nothing is ever hidden silently. Older pipeline files read fine (missing cols → '').
export const PIPELINE_COLS = ['company', 'role', 'url', 'location', 'score', 'band', 'recommendation', 'status', 'posted', 'first_seen', 'updated', 'prescreen', 'screen_reason']
...
export function recordPrescreen(existing, url, { score, reason }, dateStr) {
  let hit = false
  const out = (existing || []).map((r) => {
    if (r.url !== url) return r
    hit = true
    return { ...r, prescreen: String(Math.round(Number(score) || 0)), screen_reason: reason || '', updated: dateStr }
  })
  return hit ? out : null
}
