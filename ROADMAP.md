...
> **Date:** 2026-09-25 (Phases 0–7 built 2026-06-05; Phase 10 direction locked + L0–L5 shipped
> 2026-07-08; status banner refreshed 2026-09-25)

---

## Known gaps & current limitations

Current as of v1.63.2 — tracked deliberately so they aren't mistaken for bugs. The app-side items are elaborated in
...
- **The evaluator's calibration is young.** The 2026-08-28 audit measured the failure on real data:
  ...
  1.53 made dead listings visible (`jobdar recheck` + scan-side liveness), and 1.54 opened the label
  funnel to the CLI (`jobdar feedback`). Still true: thresholds recalibrate from **N≥50–100 real
...
One binary, simple subcommands — **`jobdar <command>`** — on your PATH via the one-line installer or
`npm link` from a checkout today; `npm i -g jobdar` / `npx jobdar` arrive with the 1.0 npm publish (7.5):
...
tailoring, `pipeline`'s eval step) use the configured **inference backend** — the on-device model by default
([Phase 8b](#phase-8--pluggable-inference-8b-on-device-via-winccpp-first-then-8a-byo-key-auto-eval), shipped) or your own API key (8a).
...
| 0.2 🔶 | **Name locked: "Jobdar"** (screened clear on npm + jobs/HR products, 2026-06-05; renamed "Jobfaro" 2026-07-16, **reverted 2026-09-25** — jobfaro.com had become a live third-party business and prospective users preferred Jobdar; npm `jobdar` + jobdar.app/.io/.ai re-verified free that day). Final confirm: formal USPTO/EUIPO trademark search; grab `jobdar.app`/`.io`/`.ai` — the web app is branded jobdar.ai (the `.com` is registered elsewhere since 2014; not ours) + a GitHub org (`/jobdar` is taken — use e.g. `getjobdar`/`jobdar-app`). | — |
...
| 5.5.4 | **Workday quirk tenants** — `allina`, `methodisthealthsystem` (HTTP 500) and `hca` (HTTP 422) reject the standard CXS POST even with a browser UA; needs deeper request replication (cookies/calendar token); 2026-09-25: ~20 Northeast/West health systems + retailers (Kaiser, Providence, Nordstrom, Costco, MGB, Northwell, UPMC, Penn Medicine…) also 422 — but with **guessed** site slugs; first resolve the real slug from each tenant's landing page, then decide quirk vs. wrong path. | 🔶 open (diagnosed: not UA-gating) |
...
> `1.41.0-jobdar.1` on engine b11146 (canary + eval bench re-validated 2026-09-24: 5/11/34 identical to the
> 08-28 baseline, 0 flips — see CHANGELOG 1.61.1).
...
> **Dependency: the `winc-jobdar` BRANCH** (newest `1.41.0-jobdar.1`; **winc ≥ 1.40.0 is required** for engine install/update after llama.cpp's 2026-08-21 release-scheme change), NOT master — its **`winc serve
...
4. **Regions to seed after Midwest.** *(Resolved 2026-09-25 — 1.63.2 added Northeast 21 + West 25, live-verified; the honest gap is that big Workday health systems/retailers in both regions answer the public CXS call with 422 under guessed site names — a 5.5.4-class item.)* → *Recommend:* let early user demand decide; `nationwide`/remote works
...
| L6 | TestFlight: eas.json profiles ready; remaining = Sam's account steps (App Store Connect record as Jobdar/com.jobdar.app, `eas login` + `eas init` [the pre-rename projectId was removed], icon choice) + first internal build | ready for Sam |
...
**Fun is a product principle (Sam, 2026-07-09):** Jobdar should feel fun to use, never a chore. The
