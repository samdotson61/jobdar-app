...
Run `node bin/jobfaro doctor` first — it checks Node, config, and providers, and tells you what's
optional.
...
| Scan returns few or no roles | Your level/region filters may be narrow. Try `jobfaro scan --levels entry,mid,senior` or `--regions nationwide`. |
| One employer returns nothing | iCIMS is best-effort; some sites are JS-rendered — try `jobfaro scan --playwright` (needs Playwright). Workday may need the right `site:` in the portal. |
...
Still stuck? Open an issue and include your `jobfaro doctor` output.
