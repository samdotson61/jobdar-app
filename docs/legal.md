...
Jobdar is provided "as is", without warranty of any kind (see the Apache-2.0 `LICENSE`). You are
...
- **Respect Terms of Service & robots.** Jobdar fetches public, browser-accessible job endpoints with
  polite pacing and per-request timeouts. Don't repurpose it to hammer a site or bypass access controls.
- **Human-in-the-loop.** Jobdar drafts and recommends; it **never auto-applies** or submits anything on
  your behalf. You review and apply.
- **Public endpoints only.** Jobdar uses unauthenticated public job data. The optional iCIMS Job Portal
