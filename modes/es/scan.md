...
`providers/`. Tú no descargas páginas de empleo; ejecutas `jobfaro scan` (o `node scan.mjs`) y
...
- `jobfaro scan --dry-run` resuelve un proveedor por portal e imprime un resumen **sin llamadas de
  red** — úsalo para revisar la configuración.

## Tu papel como agente
- Ayuda a la persona a añadir empleadores — `jobfaro seed --region <r> --write` los materializa
...
- Indica a la persona el panel para una vista rápida — `jobfaro tui` (terminal) o
