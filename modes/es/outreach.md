...
- **Cortés por construcción.** La cadencia de abajo la aplica `jobfaro outreach` en código —
  trabaja con ella, nunca en su contra.

## El flujo (herramienta determinista + tú)

1. **Encuentra a la persona.** `jobfaro outreach <urlDelPuesto>` imprime enlaces de búsqueda de
...
4. **Pasa el lint antes de mostrar la versión final.** Ejecuta `jobfaro outreach --lint
...
5. **La persona usuaria lo envía** y luego lo registra:
   `jobfaro outreach --log --url <urlDelPuesto> --person "Nombre" [--channel linkedin|email]`.

## Cadencia (aplicada por la herramienta — explícala, no la pelees)
- Máximo **2 personas por puesto**; un hilo por persona.
- **Un** seguimiento cortés por persona, maduro tras **≥5 días hábiles** — `jobfaro outreach
...
`jobfaro outreach --log` para que el momento del seguimiento quede registrado.
